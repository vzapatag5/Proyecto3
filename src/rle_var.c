    ...
    *out = NULL; *out_len = 0;
    ...
    while (i < in_len) {
        ...
        if (run >= 3) {
            ...
            // construir bloque literal hasta que aparezca run >=3 o llegue a 127
            uint8_t lit[127];
            size_t n = 0;
            lit[n++] = in[i++]; // primer byte
            // intenta añadir el segundo si no genera run >=3 inmediatamente
            while (i < in_len && n < 127) {
                // ¿hay run a partir de i?
                size_t r = 1;
...
int rle_var_decompress(const uint8_t* in, size_t in_len, uint8_t** out, size_t* out_len) {
    *out = NULL; *out_len = 0;
    ...
    // primera pasada para estimar tamaño
    size_t est = 0, i = 0;
    while (i < in_len) {
        uint8_t hdr = in[i++];
        if (hdr & 0x80) { // run
